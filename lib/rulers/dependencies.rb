...
class Object
  def self.const_missing(c)
    ...
    if penultimate_caller == "const_missing"
      raise "Found file #{Rulers.to_snakecase(c.to_s)}.rb, but failed to find #{c.to_s} in it (is either the filename or classname misspelled?)."
    end

    require Rulers.to_snakecase(c.to_s)
    Object.const_get(c)
  end
end